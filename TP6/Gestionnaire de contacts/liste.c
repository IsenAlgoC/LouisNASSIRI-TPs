...
int InsertElementAt(LinkedList *Liste, int i, Enregistrement pers) {
	...
	if (CurrentElement != NULL) {
		...
		if (Liste->size == 0) { // insertion en tête de l'unique élément
			NewElement = NewLinkedListElement(pers);
			if (NewElement != NULL) {
				Liste->head = NewElement;  //notre tete deviens notre element
				Liste->tail = NewElement;   //notre queue aussi car on a qu'un seul element 
				NewElement->next = NULL;
				Liste->size = 1;           //la taille passe donc de 0 a 1
				return 1;
			}
			else {
				return(0);
		...
		if (Liste->size <= i) { // insertion en queue
			NewElement = NewLinkedListElement(pers);
			if (NewElement != NULL) {
				Liste->tail->next = NewElement; //alors la queue actuelle de la liste pointe vers le nouvel element
				Liste->tail = NewElement;       // la queue prend la valeur du nouvel element
				NewElement->next = NULL;
				Liste->size += 1;
				return 1;
			}
...
int DeleteLinkedListElem(LinkedList * list, SingleLinkedListElem * item) {
	...
	if (item == list->head) {    //si l'element a supprimer est la tete de la litste
		list->head = item->next;//la tete prend la valeur de l'elemeny
		list->size -= 1;        //taille liste reduite
		free(item);             //on libere l'espace en memoire
		return 1;
	}
	...
	// S'il est au milieu on supprime l'element
	if (previous != NULL && tmp == item && tmp->next != NULL) {
		previous->next = item->next;
		list->size--; // la taille liste est reduite
		free(item);   //on libere l'espace en memoire
		return 1;
	}
	return(0);  // pas trouvé
